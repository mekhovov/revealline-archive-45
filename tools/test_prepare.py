...
class CommittedMetadataTests(unittest.TestCase):
    def test_exact_original_metadata_and_source_qualification(self):
        ...
        self.assertEqual((lock['expectedFiles'], lock['expectedBytes']), (886, 423409020))
        self.assertEqual([r['version'] for r in lock['releases']], ['v0.80.0'])
        self.assertEqual(inventory['base'], 'https://mekhovov.github.io/revealline-archive-45/')
        release = lock['releases'][0]
        self.assertEqual(release['tagObject'], '52b2b2f7bd248357bfae88f1ec5b0b072bf22185')
        self.assertEqual(release['sourceRevision'], '9dc3af75801517efe4a9f52266ffef6d37cb77e9')
        self.assertEqual(release['sourceTree'], 'f830c0f56a3e497e5a77168e5a803a9f48419f97')
        self.assertEqual(release['sourceQualification'], {'bytes': 138292, 'sha256': '4877d093d345956b8802680ce2c97554a2f2d49cdd30bd6e71c62f07c606dcde'})
        cohort = [r for r in inventory['files'] if r['path'].startswith('releases/v0.80.0/')]
        self.assertEqual((len(cohort), sum(r['bytes'] for r in cohort)), (883, 423407626))
